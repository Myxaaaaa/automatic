...
- В меню есть пункт «Сканировать QR платформы».
- Ожидаемый формат QR (JSON):
```
{"endpoint":"http://<HOST>:3001/notify","secret":"test-secret"}
```
- После сканирования приложение сохранит URL и секрет и будет слать заголовок `Authorization: Bearer <secret>`.
